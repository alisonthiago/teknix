      ...
      <div className="bg-white border border-[#e6e6e6] rounded-3xl p-6 shadow-sm">
        <div className="flex items-center justify-between pb-4 mb-6 border-b border-[#f1f5f9]">
          <div>
            ...
            <p className="text-xs text-[#666666] mt-0.5">Rastreabilidade completa desde a venda até a entrega final.</p>
          </div>
            ...
            return (
              <div key={i} className="relative flex items-start gap-4">
                <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 z-10 shadow-2xs border ${
                  st.done ? 'bg-[#2563eb] text-white border-[#1d4ed8]' : 'bg-[#F7F7F7] text-[#999999] border-[#e2e8f0]'
                }`}>
                  <Icon className="w-4 h-4" />
                </div>

                <div className="flex-1 min-w-0 pt-0.5">
                  <p className={`text-sm font-bold leading-tight ${st.done ? 'text-[#111111]' : 'text-[#999999]'}`}>
                    {st.title}
                  </p>
                  <p className="text-xs text-[#666666] mt-0.5">{st.desc}</p>
                </div>
              </div>
            )
          })}
            ...
            {order.timeline.map((h, i) => (
              <div key={i} className="p-3 flex items-start gap-3">
                ...
                <div className="flex-1">
                  <span className="text-xs font-semibold text-[#111111]">{h.description}</span>
                </div>
                <div className="text-sm text-[#999999] font-mono text-right shrink-0">
                  {h.date} {h.time}
                </div>
              </div>
            ))}
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
              <div>
                ...
                <div className="flex flex-wrap items-center gap-2.5 mt-2">
                  ...
                  >
                    <User className="w-3.5 h-3.5 text-[#1f2328] group-hover:text-[#0071e3]" />
                    <span className="text-[#666666] font-medium">Cliente:</span>
                    <strong className="text-[#111111] group-hover:text-[#0071e3] font-extrabold text-[13px] group-hover:underline underline-offset-2">
                      {order.customer.name}
                    </strong>
                  </Link>
