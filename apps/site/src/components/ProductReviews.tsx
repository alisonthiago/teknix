...
export function RatingSummary({ reviews }: { reviews: ProductReview[] }) {
  const {count, average} = reviewSummary(reviews)
  return <a className="rating-summary" href="#reviews"><span aria-hidden="true">★</span> {count ? average.toFixed(1).replace('.', ',') : 'Sem avaliações'} {count > 0 && `(${count} avaliações)`}</a>
}
  ...
  return <section className="product-reviews ui container" id="reviews">
    ...
    {!count ? <p>Este produto ainda não recebeu avaliações.</p> : <div className="reviews-layout">
      <div className="reviews-overview">
        ...
        <div className="review-recommendation"><strong>{Math.round(valid.filter(r=>r.recommended).length / count * 100)}%</strong><span>recomendam este produto</span></div>
      </div>
