...
function getStockStatus(stock?: number) {
  if (stock === undefined) return { label: '', class: '' }
  if (stock <= 0) return { label: 'INDISPONÍVEL', class: 'out-of-stock' }
  if (stock <= 5) return { label: 'ÚLTIMAS UNIDADES', class: 'low-stock' }
  ...
  return (
    <Link to={`/produtos/${product.slug || product.id}`} className="product-card">
      <div className="product-image">
        ...
        {stockStatus.class && (
          <span className={`product-badge badge-stock ${stockStatus.class}`}>
            {stockStatus.label}
          </span>
        )}
      </div>
      ...
      <button className="product-whatsapp" onClick={openWhatsApp}>
