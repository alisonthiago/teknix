...
import {
  ...
  Copy, Sparkles, FileText, Image as ImageIcon, Search, RefreshCw, ArrowLeft
} from 'lucide-react'
import DisplayConditionsModal, { DisplayCondition } from '../components/editor/DisplayConditionsModal'
import type { HeaderModel } from '../components/editor/GlobalHeaderRenderer'
import '../components/editor/ThemeBuilderModal.css'
...
const INITIAL_THEME_PARTS: ThemePartItem[] = [
  {
    id: 'header-global-apple-dark',
...
export default function ThemeBuilderPage() {
  ...
  const [parts, setParts] = useState<ThemePartItem[]>(INITIAL_THEME_PARTS)
  const [searchTerm, setSearchTerm] = useState('')
  const [showAddNewModal, setShowAddNewModal] = useState(false)
  const [newPartType, setNewPartType] = useState<ThemePartType>('header')
  const [newPartName, setNewPartName] = useState('')
  const [editingConditionsItem, setEditingConditionsItem] = useState<ThemePartItem | null>(null)
  ...
  const handleEditPart = (part: ThemePartItem) => {
    // Abre no editor com foco isolado na parte do tema
    navigate(`/editor/page/home-official-teknix?mode=theme_part&part_type=${part.type}&model=${part.modelId || 'apple_dark'}`)
  }

  return (
    <div className="theme-builder-page-container">
      {/* ── TOPBAR ── */}
          ...
          <div className="tb-cards-grid">
            {filteredParts.map(item => (
              <div key={item.id} className="tb-part-card">
                ...
                {/* Visual Preview Box */}
                <div className="tb-card-preview" onClick={() => handleEditPart(item)}>
                  {item.type === 'header' && (
                  ...
                  <div className="tb-card-hover-overlay">
                    <button type="button" className="tb-edit-action-btn" onClick={() => handleEditPart(item)}>
                      <Edit3 size={13} />
                ...
                <div className="tb-card-footer">
                  <div className="tb-instance-info">
                    ...
                    <button
                      type="button"
                      className="tb-btn-edit-conditions"
                      onClick={() => setEditingConditionsItem(item)}
                      title="Editar Condições de Exibição"
                    >
                      Condições
                    </button>
                  </div>
      ...
      {editingConditionsItem && (
        <DisplayConditionsModal
          isOpen={!!editingConditionsItem}
          onClose={() => setEditingConditionsItem(null)}
          onSave={(conditions) => {
            const isEntire = conditions.some(c => c.type === 'include' && c.target === 'entire_site')
            const isNo = conditions.length === 0 || conditions.every(c => c.type === 'exclude')
            const newInstance = isEntire ? 'Entire site' : isNo ? 'No instances' : `${conditions.length} condições ativas`
            setParts(parts.map(p => p.id === editingConditionsItem.id ? { ...p, instance: newInstance, status: isNo ? 'draft' : 'published' } : p))
            setEditingConditionsItem(null)
          }}
          modelName={editingConditionsItem.name}
          initialConditions={
            editingConditionsItem.instance === 'Entire site'
              ? [{ id: '1', type: 'include', target: 'entire_site' }]
              : [{ id: '1', type: 'include', target: 'singular', subTarget: 'all_pages' }]
          }
        />
      )}
