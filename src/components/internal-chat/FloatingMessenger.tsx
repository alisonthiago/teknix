...
export default function FloatingMessenger() {
    ...
    return (
      <div className="fixed bottom-4 right-4 z-[90] flex items-center gap-2">
        <button
          ...
          className="px-4 py-2.5 bg-[#111] hover:bg-[#222] text-white rounded-full shadow-2xl flex items-center gap-2.5 cursor-pointer border border-[#333] transition-all hover:scale-105"
        >
          ...
          <span className="text-xs font-bold truncate max-w-[140px]">{activeConversation?.name || 'Chat Interno'}</span>
        </button>
        <button
          onClick={() => setIsFloatingOpen(false)}
          className="w-8 h-8 rounded-full bg-white border border-[#e2e8f0] flex items-center justify-center text-[#64748b] hover:text-[#111] shadow-lg cursor-pointer transition-colors"
        >
          <X className="w-4 h-4" />
        </button>
      </div>
  ...
  return (
    <div className="fixed bottom-4 right-4 z-[90] w-80 sm:w-96 h-[500px] bg-white rounded-2xl shadow-2xl border border-[#e2e8f0] flex flex-col overflow-hidden animate-in slide-in-from-bottom-5 duration-200">
      {/* Top Bar / Header do Messenger */}
        ...
        <div className="relative">
          ...
          >
            ...
            <div className="min-w-0">
              ...
              <p className="text-[10px] text-[#16a34a] font-medium leading-none mt-0.5">Online agora</p>
            </div>
          </button>

          {/* Menu Dropdown de Seleção de Conversa */}
          {showChannelSelect && (
            <div className="absolute top-12 left-0 w-64 bg-white text-[#111] rounded-xl shadow-2xl border border-[#e2e8f0] p-1.5 z-50 divide-y divide-[#f1f5f9]">
              <p className="text-[10px] font-black uppercase text-[#94a3b8] px-2 py-1 tracking-wider">Conversas & Grupos</p>
              {conversations.map(c => (
                <button
                  key={c.id}
                  onClick={() => {
        ...
        <div className="flex items-center justify-between text-[#64748b] px-1">
          <div className="flex items-center gap-2">
            <button 
              ...
              className="p-1 hover:text-[#111] transition-colors cursor-pointer" 
              title="Enviar Foto"
            >
          ...
          <span className="text-[10px] text-[#94a3b8]">Enter para enviar</span>
        </div>
      </div>
    </div>
  )
